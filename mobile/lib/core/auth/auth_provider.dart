  ...
  Future<void> _checkAuthStatus() async {
    try {
      ...
      if (isAuthenticated) {
        ...
        final courierId = await storage
            .read(key: StorageKeys.courierId)
            .timeout(const Duration(seconds: 2), onTimeout: () => null);
        final courierPhone = await storage
            .read(key: StorageKeys.courierPhone)
            .timeout(const Duration(seconds: 2), onTimeout: () => null);
...
class AuthService {
  ...
  Future<bool> login(String phone, String password) async {
    try {
      ...
      final normalizedPhone = _normalizeCameroonPhone(phone);

      final response = await _dio.post(
        '/api/auth/token/',
        data: {'phone_number': normalizedPhone, 'password': password},
      );
      ...
      final message = _dioErrorMessage(e, 'Identifiants incorrects');
      _ref.read(authStateProvider.notifier).setError(message);
      return false;
    }
  }
  ...
  String _normalizeCameroonPhone(String phone) {
